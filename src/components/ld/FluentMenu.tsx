...
export const FluentMenuTrigger: React.FunctionComponent<FluentMenuTriggerProps> = (props) => {
  ...
  return React.cloneElement(children, {
    ...
    onClick(event: React.MouseEvent<HTMLElement>) {
      (children.props as Record<string, unknown>).onClick?.(event);
      if (isOpen) {
        onClose();
      } else {
        focusNone();
    ...
    onKeyDown(event: React.KeyboardEvent<HTMLElement>) {
      (children.props as Record<string, unknown>).onKeyDown?.(event);
      const {key} = event;
...
export const FluentMenuList: React.FunctionComponent<FluentMenuListProps> = (props) => {
  ...
  const mappedChildren = React.Children.map(children, (child) => {
    ...
    return React.cloneElement(child as React.ReactElement<Record<string, unknown>>, {
      onClick(event: React.MouseEvent<HTMLElement>) {
        (child.props as Record<string, unknown>).onClick?.(event);
        onClose();
      },
