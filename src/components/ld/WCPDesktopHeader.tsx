...
function CartIcon({ count = 0, price = '$0.00', textColor = 'white' }: { count?: number; price?: string; textColor?: string }) {
  return (
    <div className="ld-wcp-desktop-header-cart-wrap">
      <div className="ld-wcp-desktop-header-cart-icon-wrap">
        ...
        <div className="ld-wcp-desktop-header-cart-badge">
          {count > 99 ? '99+' : count}
        </div>
      </div>
      <div className="ld-wcp-desktop-header-cart-price" style={{ color: textColor }}>
        {price}
      </div>
    </div>
  );
...
const HeartSvg = () => (
  <svg width="20" height="20" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    <path fill="currentColor" d="M21.62 4.041a8.344 8.344 0 0 1 5.92 2.493 8.573 8.573 0 0 1 2.46 6v.162c0 3.142-1.84 6.487-5.48 9.831a40.81 40.81 0 0 1-7.06 5.088 2.969 2.969 0 0 1-2.92 0 40.806 40.806 0 0 1-7.06-5.088C3.84 19.183 2 15.88 2 12.697v-.163a8.56 8.56 0 0 1 1.346-4.64 8.392 8.392 0 0 1 3.629-3.137 8.26 8.26 0 0 1 4.732-.615A8.315 8.315 0 0 1 16 6.25 8.3 8.3 0 0 1 21.62 4v.041Zm-10.219 2.06a6.283 6.283 0 0 0-3.622.467 6.386 6.386 0 0 0-2.771 2.41A6.511 6.511 0 0 0 4 12.534v.162c0 2.92 2.12 5.838 4.82 8.331a39.015 39.015 0 0 0 6.7 4.825 1.009 1.009 0 0 0 .96 0 39.008 39.008 0 0 0 6.7-4.825c2.7-2.493 4.82-5.412 4.82-8.33v-.163a6.51 6.51 0 0 0-1.868-4.572 6.342 6.342 0 0 0-4.196-1.887l-.316-.048a6.333 6.333 0 0 0-4.3 1.702l-.58.548a1.112 1.112 0 0 1-1.48 0l-.58-.548a6.327 6.327 0 0 0-3.28-1.627Z"/>
  </svg>
);
...
const UserSvg = () => (
  <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    ...
    <path d="M4 17C4 14.2386 6.23858 12 9 12H11C13.7614 12 16 14.2386 16 17" stroke="currentColor" strokeWidth="1.5" strokeLinecap="square" />
  </svg>
);
...
function TypeaheadSeparator() {
  return <div className="ld-wcp-desktop-header-typeahead-separator" />;
}
...
function TypeaheadHorizontalScrollSection({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="ld-wcp-desktop-header-typeahead-scroll-row">
      {canScrollLeft && (
        <button
          type="button"
          className="ld-wcp-desktop-header-typeahead-scroll-button"
      ...
      <div ref={scrollRef} className="ld-wcp-desktop-header-typeahead-scroll-list">
        {children}
      </div>
...
interface WCPDesktopHeaderProps {
  cartCount?: number;
  cartPrice?: string;
}

export function WCPDesktopHeader({ cartCount: cartCountProp, cartPrice: cartPriceProp }: WCPDesktopHeaderProps = {}) {
  ...
  const cartCount = cartCountProp ?? 3;
  const cartPrice = cartPriceProp ?? '$15.00';
  ...
  const gicSubtext = selectedFulfillment === 'none' ? (
    <>
      ...
      <span className="ld-wcp-desktop-header-gic-subtext-dot">&bull;</span>
      <span className="ld-wcp-desktop-header-gic-subtext-ellipsis">Sunnyvale Supercenter</span>
    </>
  ) : selectedFulfillment === 'pickup' ? (
    <span className="ld-wcp-desktop-header-gic-subtext-ellipsis">Carrollton Supercenter</span>
  ) : (
    <span>3471 Park Ln</span>
  );

  const gicIconContent = selectedFulfillment === 'none' ? (
    <img src={GIC_ICON} alt="Global Intent" width="40" height="40" className="ld-wcp-desktop-header-gic-icon-img" />
  ) : selectedFulfillment === 'shipping' ? (
    <FulfillmentShippingSvg />
  ) : selectedFulfillment === 'pickup' ? (
    <img src={PICKUP_ICON} alt="Pickup" width="40" height="40" className="ld-wcp-desktop-header-gic-icon-img" />
  ) : (
    <img src={DELIVERY_ICON} alt="Delivery" width="40" height="40" className="ld-wcp-desktop-header-gic-icon-img" />
  );
  ...
  return (
    <header className="ld-wcp-desktop-header-root">
      <div className="ld-wcp-desktop-header-content">
        {/* Walmart Logo */}
        <a
          href="#"
          className="ld-wcp-desktop-header-logo"
          aria-label="Walmart Homepage"
        >
          <img src={SPARK_ICON} alt="Walmart" width="36" height="36" className="ld-wcp-desktop-header-logo-img" />
        </a>

        {/* GIC: Location / Delivery selector */}
        <div className="ld-wcp-desktop-header-gic-wrap" ref={gicDropdownRef}>
          <div className={`ld-wcp-desktop-header-gic-section${gicOpen ? ' ld-wcp-desktop-header-gic-section-active' : ''}`}>
            <button
              type="button"
              className="ld-wcp-desktop-header-gic-button"
              onClick={() => openDropdown(gicOpen ? null : 'gic')}
            >
              <div className="ld-wcp-desktop-header-gic-icon">
                {gicIconContent}
              </div>
              <div className="ld-wcp-desktop-header-gic-text">
                <div className="ld-wcp-desktop-header-gic-label">{gicLabel}</div>
                <div className="ld-wcp-desktop-header-gic-subtext">
                  {gicSubtext}
                </div>
              </div>
              <div className={`ld-wcp-desktop-header-gic-chevron${gicOpen ? ' ld-wcp-desktop-header-gic-chevron-open' : ''}`}>
                <i className="ld ld-ChevronDown ld-wcp-desktop-header-gic-chevron-icon" />
              </div>
            </button>
          </div>
          ...
          {showHighlight && !gicOpen && (
            <div
              role="status"
              aria-live="polite"
              className="ld-wcp-desktop-header-highlight"
            >
              <div className="ld-wcp-desktop-header-highlight-nubbin" />
              <div className="ld-wcp-desktop-header-highlight-icon">
              ...
              <div className="ld-wcp-desktop-header-highlight-text">
                Is this the right location?
              </div>
              <button
                onClick={() => setShowHighlight(false)}
                className="ld-wcp-desktop-header-highlight-update-btn"
              >
                Update
              </button>
              <button
                onClick={() => setShowHighlight(false)}
                className="ld-wcp-desktop-header-highlight-close-btn"
              >
          ...
          {gicOpen && (
            <>
              <div className="ld-wcp-desktop-header-scrim" onClick={() => openDropdown(null)} />
              <div className="ld-wcp-desktop-header-gic-dropdown">
                {/* Fulfillment options section */}
                <div className="ld-wcp-desktop-header-gic-options-section">
                  <div className="ld-wcp-desktop-header-gic-options-list">
                    {fulfillmentOptions.map((opt) => (
                      <button
                        key={opt.key}
                        type="button"
                        className="ld-wcp-desktop-header-gic-option-btn"
                      ...
                      >
                        <div className="ld-wcp-desktop-header-gic-option-icon-wrap">
                          {opt.icon}
                          {selectedFulfillment === opt.key && (
                            <>
                              <svg
                                className="ld-wcp-desktop-header-gic-selection-ring"
                                viewBox="0 0 64 64"
                                fill="none"
                              ...
                              <button
                                ...
                                className="ld-wcp-desktop-header-gic-deselect-btn"
                                aria-label={`Deselect ${opt.label}`}
                              >
                                <span className="ld-wcp-desktop-header-gic-deselect-badge">
                                  <svg className="ld-wcp-desktop-header-gic-deselect-circle" viewBox="0 0 21 21" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                                    <circle cx="10.5" cy="10.5" r="10.5" />
                                  </svg>
                                  <span className="ld-wcp-desktop-header-gic-deselect-x" aria-hidden="true">
                        ...
                        <span className="ld-wcp-desktop-header-gic-option-label">{opt.label}</span>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                {/* Address & store cards */}
                <div className="ld-wcp-desktop-header-gic-cards-section">
                  <div className="ld-wcp-desktop-header-gic-cards-list">
                    <button type="button" className="ld-wcp-desktop-header-gic-card">
                      <LocationSvg />
                      <span className="ld-wcp-desktop-header-gic-card-text-single">
                    ...
                    <button type="button" className="ld-wcp-desktop-header-gic-card">
                      <StoreSvg />
                      <div className="ld-wcp-desktop-header-gic-card-text-stack">
                        <span className="ld-wcp-desktop-header-gic-card-store-name">Carrollton Supercenter</span>
                        <span className="ld-wcp-desktop-header-gic-card-store-addr">1213 E Trinity Mills Rd, Dallas, TX 75220</span>
                      </div>
                      <ChevronRightSvg />
                    </button>
                  </div>
        ...
        <div
          ref={searchFormRef}
          className={`ld-wcp-desktop-header-search-form-wrap${searchOpen ? ' ld-wcp-desktop-header-search-form-wrap-focused' : ''}`}
        >
          <form
            className="ld-wcp-desktop-header-search-form"
            role="search"
          ...
          >
            <div className={`ld-wcp-desktop-header-search-input-wrap${searchOpen ? ' ld-wcp-desktop-header-search-input-wrap-active' : ''}`}>
              <input
                type="search"
                name="q"
                ...
                className="ld-wcp-desktop-header-search-input"
              />
              <div className="ld-wcp-desktop-header-search-actions">
                {searchQuery.trim() ? (
                  <button
                    type="button"
                    className="ld-wcp-desktop-header-search-clear-button"
                  ...
                  <button
                    type="button"
                    className="ld-wcp-desktop-header-search-button"
                    aria-label="Search"
                    onClick={() => {
                      setSearchOpen(true);
                    }}
                  >
                    <i className="ld ld-Search ld-wcp-desktop-header-search-icon" />
                  </button>
                )}
              </div>
            </div>
          ...
          {searchOpen && (
            <>
              <div className="ld-wcp-desktop-header-search-scrim" onClick={closeSearchResults} />
              <div className="ld-wcp-desktop-header-search-results" role="listbox" aria-label="Search suggestions">
                {searchQuery.trim() ? (
                  filteredSuggestions.length > 0 ? (
                    filteredSuggestions.map((suggestion, index) => {
                      ...
                      return (
                        <button
                          ...
                          type="button"
                          className="ld-wcp-desktop-header-search-result-row"
                          onClick={() => {
                            setSearchQuery(suggestion);
                            handleSearchSubmit(suggestion);
                          }}
                        >
                          <span className="ld-wcp-desktop-header-search-result-text">
                            {parts.before && <span className="ld-wcp-desktop-header-search-result-strong">{parts.before}</span>}
                            <span>{parts.match || suggestion}</span>
                            {parts.after && <span className="ld-wcp-desktop-header-search-result-strong">{parts.after}</span>}
                          </span>
                          <span className="ld-wcp-desktop-header-search-result-icon">
                            <ArrowUpLeftSvg />
                          </span>
                        </button>
                      );
                    ...
                    <div className="ld-wcp-desktop-header-search-empty-state">
                      No matching suggestions found.
                    </div>
                  )
                ) : (
                  <>
                    <section className="ld-wcp-desktop-header-typeahead-section">
                      <h3 className="ld-wcp-desktop-header-typeahead-section-heading">
                      ...
                      <TypeaheadHorizontalScrollSection>
                        {KEEP_SHOPPING_FOR.map((item) => (
                          <button
                            key={item.label}
                            type="button"
                            className="ld-wcp-desktop-header-typeahead-tile-button"
                            onClick={() => handleChipClick(item.label)}
                          >
                            {item.isFilter ? (
                              <div className="ld-wcp-desktop-header-typeahead-tile-filter">
                                <span className="ld-wcp-desktop-header-typeahead-tile-filter-label">{item.label}</span>
                              </div>
                            ) : (
                              <div className="ld-wcp-desktop-header-typeahead-tile-image">
                                <img src={item.image ?? ''} alt={item.label} className="ld-wcp-desktop-header-typeahead-tile-image-img" />
                              </div>
                            )}
                            <span className="ld-wcp-desktop-header-typeahead-tile-label">{item.label}</span>
                          </button>
                        ))}
                      </TypeaheadHorizontalScrollSection>
                    </section>
                    ...
                    <section className="ld-wcp-desktop-header-typeahead-section-padded">
                      <div className="ld-wcp-desktop-header-typeahead-section-heading-row">
                        <h3 className="ld-wcp-desktop-header-typeahead-section-heading" style={{ margin: 0 }}>
                          Grab your usuals
                        </h3>
                        <button type="button" className="ld-wcp-desktop-header-typeahead-my-items-link">
                          <span className="ld-wcp-desktop-header-typeahead-my-items-icon">
                            <HeartSvg />
                          </span>
                          My items
                        </button>
                      ...
                      <TypeaheadHorizontalScrollSection>
                        {GRAB_YOUR_USUALS.map((term) => (
                          <button
                            key={term}
                            type="button"
                            className="ld-wcp-desktop-header-typeahead-chip-button"
                            onClick={() => handleChipClick(term)}
                          >
                            <span className="ld-wcp-desktop-header-typeahead-chip-label">{term}</span>
                          </button>
                        ))}
                      </TypeaheadHorizontalScrollSection>
                    </section>
                    ...
                    {recentSearches.length > 0 && (
                      <>
                        <section className="ld-wcp-desktop-header-typeahead-section-padded">
                          <h3 className="ld-wcp-desktop-header-typeahead-section-heading">Your recent searches</h3>
                          <div>
                            {recentSearches.map((search, index) => (
                              <React.Fragment key={`${search}-${index}`}>
                                <div className="ld-wcp-desktop-header-search-recent-row">
                                  <span className="ld-wcp-desktop-header-search-recent-icon">
                                    <HistorySvg />
                                  </span>
                                  <button
                                    type="button"
                                    className="ld-wcp-desktop-header-search-recent-term"
                                  ...
                                  <button
                                    type="button"
                                    className="ld-wcp-desktop-header-search-recent-remove"
                    ...
                    <section className="ld-wcp-desktop-header-typeahead-section-padded">
                      <h3 className="ld-wcp-desktop-header-typeahead-section-heading">Trending</h3>
                      <TypeaheadHorizontalScrollSection>
                    ...
                    <section className="ld-wcp-desktop-header-typeahead-section-padded">
                      <h3 className="ld-wcp-desktop-header-typeahead-section-heading">Your frequent searches</h3>
                      <TypeaheadHorizontalScrollSection>
        ...
        {/* Navigation */}
        <nav aria-label="Account and Cart" className="ld-wcp-desktop-header-nav">
          <ul className="ld-wcp-desktop-header-nav-list">
            {/* Reorder / My Items */}
            <li>
              <div className="ld-wcp-desktop-header-dropdown-root" ref={reorderDropdownRef}>
                <button
                  type="button"
                  className="ld-wcp-desktop-header-nav-button"
                  aria-haspopup="true"
                  aria-expanded={reorderOpen}
                  onClick={() => openDropdown(reorderOpen ? null : 'reorder')}
                >
                  <HeartSvg />
                  <div className="ld-wcp-desktop-header-nav-content">
                    <div className="ld-wcp-desktop-header-nav-subtext">Reorder</div>
                    <div className="ld-wcp-desktop-header-nav-label">My Items</div>
                  </div>
                </button>

                {reorderOpen && (
                  <div className="ld-wcp-desktop-header-dropdown-panel ld-wcp-desktop-header-dropdown-panel-left" role="menu" aria-label="Reorder and My Items">
                    <button type="button" role="menuitem" className="ld-wcp-desktop-header-dropdown-item" onClick={() => openDropdown(null)}>
                      <ReorderSvg />
                      <span className="ld-wcp-desktop-header-dropdown-item-label">Reorder</span>
                    </button>
                    <button type="button" role="menuitem" className="ld-wcp-desktop-header-dropdown-item" onClick={() => openDropdown(null)}>
                      <ListsSvg />
                      <span className="ld-wcp-desktop-header-dropdown-item-label">Lists</span>
                    </button>
                    <button type="button" role="menuitem" className="ld-wcp-desktop-header-dropdown-item" onClick={() => openDropdown(null)}>
                      <GiftSvg />
                      <span className="ld-wcp-desktop-header-dropdown-item-label">Registries</span>
                    </button>
                  </div>
                )}
              </div>
            ...
            {/* Account */}
            <li>
              <div className="ld-wcp-desktop-header-dropdown-root" ref={accountDropdownRef}>
                <button
                  type="button"
                  className="ld-wcp-desktop-header-nav-button"
                  aria-haspopup="true"
                ...
                >
                  <UserSvg />
                  <div className="ld-wcp-desktop-header-nav-content">
                    <div className="ld-wcp-desktop-header-nav-subtext">Hi, Emilia</div>
                    <div className="ld-wcp-desktop-header-nav-label">Account</div>
                  </div>
                </button>

                {accountOpen && (
                  <div className="ld-wcp-desktop-header-dropdown-panel ld-wcp-desktop-header-dropdown-panel-right" role="menu" aria-label="Account navigation">
                    <button type="button" role="menuitem" className="ld-wcp-desktop-header-dropdown-item" onClick={() => openDropdown(null)}>
                      <PurchaseHistorySvg />
                      <span className="ld-wcp-desktop-header-dropdown-item-label">Purchase History</span>
                    </button>
                    <button type="button" role="menuitem" className="ld-wcp-desktop-header-dropdown-item" onClick={() => openDropdown(null)}>
                      <WalmartPlusLogoSvg />
                      <span className="ld-wcp-desktop-header-dropdown-item-label">Walmart+</span>
                    </button>
                    <button type="button" role="menuitem" className="ld-wcp-desktop-header-dropdown-item" onClick={() => openDropdown(null)}>
                      <UserSvg />
                      <span className="ld-wcp-desktop-header-dropdown-item-label">Account</span>
                    </button>
                    <button type="button" role="menuitem" className="ld-wcp-desktop-header-dropdown-item" onClick={() => openDropdown(null)}>
                      <SubscriptionSvg />
                      <span className="ld-wcp-desktop-header-dropdown-item-label">Subscriptions</span>
                    </button>
                    <button type="button" role="menuitem" className="ld-wcp-desktop-header-dropdown-item" onClick={() => openDropdown(null)}>
                      <WalmartCashSvg />
                      <span className="ld-wcp-desktop-header-dropdown-item-label">Get Walmart Cash</span>
                    </button>
                    <div className="ld-wcp-desktop-header-dropdown-separator" role="separator" />
                    <button type="button" role="menuitem" className="ld-wcp-desktop-header-dropdown-item" onClick={() => openDropdown(null)}>
                      <GlobeSvg />
                      <span className="ld-wcp-desktop-header-dropdown-item-label">Language | English</span>
                    </button>
                    <div className="ld-wcp-desktop-header-dropdown-separator" role="separator" />
                    <button type="button" role="menuitem" className="ld-wcp-desktop-header-dropdown-item" onClick={() => openDropdown(null)}>
                      <SignOutSvg />
                      <span className="ld-wcp-desktop-header-dropdown-item-label">Sign Out</span>
                    </button>
                  </div>
                )}
              </div>
            ...
            {/* Cart */}
            <li>
              <a href="#" className="ld-wcp-desktop-header-nav-button-cart" aria-label={`Cart contains ${cartCount} items`}>
...
WCPDesktopHeader.displayName = 'WCPDesktopHeader';
